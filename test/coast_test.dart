...
void main() {
  group('$CoastState', () {
    group('calculate NewSourcePage', () {
      ...
      test(
          'offset bit bigger then source page should return current source page',
          () {
      ...
      test(
          'offset much smaller then source page should calculate source page based on offset',
          () {
        expect(sut.calculateNewSourcePage(offset: 1.9, sourcePage: 3), 2);
      });

      test(
          'offset bit smaller then source page should return current source page',
          () {
    ...
    group('calculate NewTargetPage', () {
      ...
      test(
          'offset is bigger then source page and should determine new target page correctly',
          () {
        expect(sut.calculateNewTargetPage(offset: 4.6, newSourcePage: 3), 4);
      });

      test(
          'offset is smaller then source page and should determine new target page correctly',
          () {
    ...
    group('finish transition', () {
      ...
      test(
          'should finish transition in case new target page is not equal to current target page',
          () {
        sut.progress = TransitionAnimation();
        expect(
            sut.shouldFinishTransition(newTargetPage: 1, newSourcePage: null),
            true);
      });

      test(
          'should finish transition in case new source page is not equal to current source page',
          () {
        sut.progress = TransitionAnimation();
        expect(
            sut.shouldFinishTransition(newTargetPage: null, newSourcePage: 1),
            true);
      });
      ...
      test('should not finish transition in case ', () {
        sut.progress = null;
        expect(
            sut.shouldFinishTransition(
                newTargetPage: null, newSourcePage: null),
            false);
      });
    });
  ...
  group('TransitionAnimation', () {
    group('Calculate animation status', () {
      ...
      test(
          'should return forward in case newvalue is bigger then previous value',
          () {
        sut.value = 0.5;
      ...
      test(
          'should return reverse in case newvalue is bigger then previous value',
          () {
        sut.value = 0.5;
    ...
    group('Event handling', () {
      ...
      test('Should dispatch correct status update object to listener',
          () async {
        sut.value = 1.0;
